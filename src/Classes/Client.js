...
class Client {
    constructor(opts) {
        ...
        this.databaseDefaults = databaseOpts.defaults || {};
        ...
        if (!this.db.getCollection("bot")) this.db.createCollection("bot", this.databaseDefaults?.bot || {});
        if (!this.db.getCollection("users")) this.db.createCollection("users", this.databaseDefaults?.users || {});
        if (!this.db.getCollection("groups")) this.db.createCollection("groups", this.databaseDefaults?.groups || {});
    }
    ...
    _onEvents() {
        ...
        this.core.ev.on("messages.upsert", async (event) => {
            ...
            for (const message of event.messages) {
                ...
                const body = Functions.getBodyFromMsg(message);
                const self = {
                    ...this,
                    sender: {
                        jid: senderJid,
        ...
        this.core.ev.on("groups.upsert", async ([event]) => {
            await this._setGroupCache(event.id);
        ...
        this.core.ws.on("CB:notification,type:passkey_prologue_request", async (node) => {
        ...
        this.core.ws.on("CB:notification,type:crsc_continuation", async (node) => {
            await this.core.sendNode({
                tag: "ack",
                attrs: {
                    id: node.attrs.id,
    ...
    async launch() {
        ...
        if (this.usePairingCode && !this.core.authState.creds.registered) {
            if (!this.phoneNumber) console.error("phoneNumber options are required if you are using usePairingCode.");

            this.phoneNumber = this.phoneNumber.replace(/[^0-9]/g, "");
            if (!this.phoneNumber.length) console.error("Invalid phoneNumber");

            await Baileys.delay(3000);
            const code = await this.core.requestPairingCode(this.phoneNumber, this.customPairingCode);
            console.log(styleText("cyan", `Pairing Code: ${code}`));
        }
        ...
        this.sendMessage = async (jid, content, options = {}) => {
            ...
            // asepxyz baileys build expects `interactiveButtons`, not `buttons`/`nativeFlow`.
            // Without this bridge, messages fall back to the legacy buttonsMessage type,
            // which WhatsApp no longer renders as clickable in 1:1 chats (only groups).
            if (content.buttons && !content.interactiveButtons) {
                content.interactiveButtons = content.buttons.map((b) => ({
                    name: b.type === "cta_url" ? "cta_url" : "quick_reply",
                    buttonParamsJson: JSON.stringify(
                        b.type === "cta_url"
                            ? { display_text: b.displayText, url: b.url }
                            : { display_text: b.displayText, id: b.id }
                    )
                }));
                delete content.buttons;
            }
            if (content.nativeFlow && Object.keys(content.nativeFlow).length === 0) delete content.nativeFlow;
