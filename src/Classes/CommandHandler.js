...
class CommandHandler {
    ...
    load(isShowLog = true) {
        if (isShowLog) console.group(styleText("cyan", "Command Handler"));
            ...
            try {
                ...
                for (const cmd of commands) {
                    ...
                    if (isShowLog) {
                        ...
                        console.log(styleText("cyan", `Loaded ${type} - ${cmd.name}`));
                    }
                }
            } catch (error) {
                if (isShowLog) console.warn(`Failed to load ${file}: ${error}`);
            }
        }
